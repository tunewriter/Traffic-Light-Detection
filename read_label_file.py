...
def get_all_labels(input_yaml, riib=False, clip=True):
    ...
    for i in range(len(images)):
        ...
        if clip:
            for j, box in enumerate(images[i]['boxes']):
                images[i]['boxes'][j]['x_min'] = max(min(box['x_min'], 1280 - 1), 0)
                images[i]['boxes'][j]['x_max'] = max(min(box['x_max'], 1280 - 1), 0)
                images[i]['boxes'][j]['y_min'] = max(min(box['y_min'], 720 - 1), 0)
                images[i]['boxes'][j]['y_max'] = max(min(box['y_max'], 720 - 1), 0)
